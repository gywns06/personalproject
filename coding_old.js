...
fetch("http://localhost:3000/Fooditem") //fetch connects to the data server
  ...
  .then((json) => {
    ...
    $("#old1").text(json[0].name + fooddictionary[json[0].name]); //$ picks a html element, in this case it picked an element "#old1" - # means 'id' // .text puts texts inside the html element that you picked
    ...
    $("#old2").text(json[1].name + fooddictionary[json[1].name]);
    $("#old3").text(json[2].name + fooddictionary[json[2].name]);
    return json;
  })
  .then((json) => {
    json.sort(compare2);
    console.log(json);
    $("#recent1").text(json[0].name + fooddictionary[json[0].name]);
    $("#recent2").text(json[1].name + fooddictionary[json[1].name]);
    $("#recent3").text(json[2].name + fooddictionary[json[2].name]);
  });
